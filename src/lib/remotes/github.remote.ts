  ...
  async ({ repo }) => {
    ...
    return cache.getOrSet(cleaned, async () => {
      ...
      const [repoRes, pullsRes, issuesRes] = await Promise.all([
        ...
        fetch(
          `${repoUrl}/pulls?state=open&per_page=5&sort=created&direction=desc`,
          { headers }
        ),
        fetch(
          `${repoUrl}/issues?state=open&per_page=10&sort=created&direction=desc`,
          { headers }
        )
      ])
      ...
      if (pullsRes.ok) {
        ...
        openPrsCount =
          extractLastPage(pullsRes.headers.get('Link')) ?? pullsData.length
      }
