  ...
  function getWeatherIcon(weatherCode) {
    ...
    if (weatherCode >= 200 && weatherCode < 300) {
      switch (weatherCode) {
        ...
        case 210:
        case 201:
        case 202: return isDay ? thunderstormsDayRain : thunderstormsNightRain;
        case 211:
        case 212: return isDay ? thunderstormsDay : thunderstormsNight;
        case 221:
        case 231:
        case 232: return isDay ? thunderstormsDayExtremeRain : thunderstormsNightExtremeRain;
    ...
    if (weatherCode >= 300 && weatherCode < 400) {
      switch (weatherCode) {
        case 300:
        case 301:
        case 302: return isDay ? partlyDayDrizzle : partlyNightDrizzle;
        case 310:
        case 311: return isDay ? overcastDayDrizzle : overcastNightDrizzle;
        case 312:
        case 313:
        case 314:
        case 321: return isDay ? extremeDayDrizzle : extremeNightDrizzle;
    ...
    if (weatherCode >= 500 && weatherCode < 600) {
      switch (weatherCode) {
        case 500:
        case 501: return isDay ? partlyDayRain : partlyNightRain;
        case 502:
        case 503:
        case 504: return isDay ? overcastDayRain : overcastNightRain;
        case 511: return sleet;
        case 520:
        case 521:
        case 522:
        case 531: return isDay ? extremeDayRain : extremeNightRain;
        default: return rain;
      }
    }
    ...
    if (weatherCode >= 600 && weatherCode < 700) {
      switch (weatherCode) {
        case 600: return hail;
        case 601:
        case 602: return isDay ? overcastDaySnow : overcastNightSnow;
        case 611:
        case 612:
        ...
        case 620:
        case 621:
        case 622: return isDay ? extremeDaySnow : extremeNightSnow;
        default: return snow;
      }
    }
    ...
    switch (weatherCode) {
      ...
      case 741: return isDay ? fogDay : fogNight;
      case 771:
      case 781: return hurricane;
      case 800: return isDay ? clearDay : clearNight;
      case 801:
      case 802: return isDay ? cloudyDay : cloudyNight;
      case 803:
...
<div class="relative flex flex-col justify-center p-4 w-full min-w-0 min-h-0">
  <button
    class="flex flex-row items-center gap-2 w-full cursor-pointer bg-transparent border-0 p-0 text-left"
  ...
  >
    ...
    {/if}
    <div>
      <strong class="block text-2xl leading-none">{weather ? toCelsius(weather.main?.temp) : '--°C'}</strong>
      <span class="text-magma-muted text-xs">{weather?.name || 'Weather'} · {weather?.main?.humidity ?? '--'}%</span>
    </div>
  </button>

  {#if tooltipOpen && weather}
    <div
      class="absolute left-full top-0 z-50 ml-3 w-64 p-3 rounded-xl bg-magma-panel-strong border border-magma-line shadow-[0_12px_40px_rgb(0_0_0/40%)] backdrop-blur-xl text-xs leading-relaxed"
      role="dialog"
      aria-label="Weather details"
      tabindex="-1"
      onclick={(e) => e.stopPropagation()}
      onkeydown={(e) => { if (e.key === 'Escape') tooltipOpen = false; }}
    >
      <div class="flex items-center gap-2 mb-2">
        <MapPin size={14} class="text-magma-accent shrink-0" />
        <span class="font-bold">{weather.name}, {weather.sys?.country || ''}</span>
      </div>
      <hr class="border-magma-line/50 my-1.5" />
      <div class="grid grid-cols-2 gap-x-3 gap-y-1">
        <span class="text-magma-muted">Temperature</span>
        <span>{toCelsius(weather.main?.temp)}</span>
        <span class="text-magma-muted">Feels like</span>
        <span>{toCelsius(weather.main?.feels_like)}</span>
        <span class="text-magma-muted">Min</span>
        <span>{toCelsius(weather.main?.temp_min)}</span>
        <span class="text-magma-muted">Max</span>
        <span>{toCelsius(weather.main?.temp_max)}</span>
        <span class="text-magma-muted">Humidity</span>
        <span>{weather.main?.humidity}%</span>
        <span class="text-magma-muted">Wind</span>
        <span>{weather.wind?.speed}m/s at {weather.wind?.deg}°</span>
        <span class="text-magma-muted">Sky</span>
        <span>{weather.weather?.[0]?.main || ''}</span>
        <span class="text-magma-muted">Visibility</span>
        <span>{formatVisibility(weather.visibility)}</span>
        <span class="text-magma-muted">Sunrise</span>
        <span>{formatHour(weather.sys?.sunrise)}</span>
        <span class="text-magma-muted">Sunset</span>
