<script lang="ts">
  ...
  function formatTime() {
    ...
    return now.toLocaleTimeString(locale, opts);
  }

  function formatDate() {
    if (!(widget.config?.showDate ?? true)) return "";
    const opts: Intl.DateTimeFormatOptions =
      size === "large"
  ...
  function formatTimezone() {
    ...
    try {
      const short = now.toLocaleTimeString(locale, {
...
{#if size === "small"}
  <div
    class="flex flex-col items-center justify-center w-full h-full min-w-0 min-h-0 p-2"
  >
    <span class="text-center">
      {dateStr}
    </span>
    <span class="text-white font-bold leading-none text-5xl">
      {timeStr}
    </span>
  </div>
{:else if size === "medium"}
