...
export const store = {
	...
	async readSystemYaml() {
		...
		try {
			...
			if (isNodeError(error) && error.code === 'ENOENT') return stringifySystemConfig(defaultSystemConfig())
			throw error
		}
	},
...
export function mergeDashboardWithSystem(config: ConfigObject, systemConfig: ConfigObject): ConfigObject {
...
function validateWidget(widget: WidgetLike, grid: GridConfig, topLevel: boolean, seen: Set<string>) {
