<script>
  // @ts-nocheck
  import { RefreshCw, TrendingUp, TrendingDown, Minus } from '@lucide/svelte'
  ...
  {:else if state === 'content'}
    <div class="flex-1 overflow-y-auto min-h-0 px-1 pb-1">
      {#each stocks as stock}
        <div class="flex items-center px-2 py-1.5 rounded hover:bg-white/5 transition-colors">
          ...
          <div class="text-right ml-2 shrink-0">
            <div class="text-magma-text text-xs font-medium tabular-nums">
              {formatPrice(stock.price)}
            </div>
          </div>
          <div class="text-right ml-3 shrink-0 min-w-[70px]">
            {#if stock.change != null}
              {#if stock.change > 0}
                <div class="flex items-center gap-0.5 justify-end text-green-400">
                  <TrendingUp class="size-3" />
                  <span class="text-xs tabular-nums">{formatChange(stock.change)}</span>
                </div>
                <div class="text-[10px] text-green-400/80 tabular-nums text-right">
                  {formatChangePercent(stock.changePercent)}
                </div>
              {:else if stock.change < 0}
                <div class="flex items-center gap-0.5 justify-end text-red-400">
                  <TrendingDown class="size-3" />
                  <span class="text-xs tabular-nums">{formatChange(stock.change)}</span>
                </div>
                <div class="text-[10px] text-red-400/80 tabular-nums text-right">
                  {formatChangePercent(stock.changePercent)}
                </div>
              {:else}
                <div class="flex items-center gap-0.5 justify-end text-magma-muted">
                  <Minus class="size-3" />
                  <span class="text-xs tabular-nums">0.00</span>
                </div>
                <div class="text-[10px] text-magma-muted tabular-nums text-right">0.00%</div>
              {/if}
            {:else}
